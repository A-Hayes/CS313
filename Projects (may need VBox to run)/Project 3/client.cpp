...
void worker_thread_function(FIFORequestChannel* chan, BoundedBuffer* reqBuf, HistogramCollection* hc, int mBuf){
  char buf[1024];
  double resp = 0;
  char recvbuf[mBuf];
...
void file_thread_function(string filename, BoundedBuffer* reqBuf, FIFORequestChannel* chan, int mBuf){
  //create
...
int main(int argc, char *argv[]){
  ...
  FIFORequestChannel* workChannels [w];
  for(int i=0; i<w; i++){
    workChannels[i] = create_new_channel(chan);
  }
